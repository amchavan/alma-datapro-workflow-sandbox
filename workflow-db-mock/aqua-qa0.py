...
from dbmsgq import ExecutorClient, MessageQueue
import random
import dbdrwutils
...
mq = MessageQueue( 'localhost', 'msgq' )
...
response = dbdrwutils.setState( xtss, args.uid, state )
...
print(" [.] response: %s" % response)
if response == 201:
    dbdrwutils.broadcastRecipeChange( ousID=args.uid, recipe=recipe, queue=mq )
