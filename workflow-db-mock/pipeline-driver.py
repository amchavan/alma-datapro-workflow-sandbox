...
listen_to = (dbdrwutils.pipelineProcessListener % args.exec)
...
def callback( body ):
	...
	#     uid://A003/X1/X3 
	ousID = body
	dbdrwutils.setState( xtss=xtss, ousID=ousID, state="Processing" )
	...
	if failed:
		dbdrwutils.setState( xtss=xtss, ousID=ousID, state="ProcessingProblem" )
	else:
		dbdrwutils.setState( xtss=xtss, ousID=ousID, state="ReadyForReview" )
